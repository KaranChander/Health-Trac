//
//  ReadingStepOneView.swift
//   Health Track


import UIKit

class ReadingStepOneView: UIView {
    ...
    func initialSetup() {
        setupFont()
        setupColor()
        printDebug(self.frame.height)
        pickerHeight.constant = self.frame.height * 0.43
    ...
    func setupColor() {
        ...
        timeColonLabel.textColor = AppColors.greyishWhite
    }
    
    ///Date Picker Setup
    func setupDatePicker() {
        datePickerView.subviews[0].subviews[0].subviews[2].backgroundColor = AppColors.pinkishOrange
        datePickerView.selectedLabelColor = .white
        datePickerView.unselectedLabelColor = .white
